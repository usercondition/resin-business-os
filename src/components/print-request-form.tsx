...
/** Staff hub: full chrome + share button. Public: form only (opened via /public/request). */
export type PrintRequestFormMode = "staff" | "public";

const PUBLIC_REQUEST_PATH = "/public/request";
const STAFF_SUCCESS_PATH = "/request/success";
const PUBLIC_SUCCESS_PATH = "/public/request/success";

type Props = {
  mode: PrintRequestFormMode;
};

export function PrintRequestForm({ mode }: Props) {
  const router = useRouter();
  const isStaff = mode === "staff";
  const successPath = isStaff ? STAFF_SUCCESS_PATH : PUBLIC_SUCCESS_PATH;
  ...
  async function copyShareLink() {
    const url = `${window.location.origin}${PUBLIC_REQUEST_PATH}`;
    await navigator.clipboard.writeText(url);
    setMessage("Client form link copied. Share this URL — it has no app menu.");
  }
